...
DATA_DIR = PROJECT_ROOT / "web_app" / "backend" / "data"
UPLOAD_DIR = PROJECT_ROOT / "web_app" / "backend" / "uploads"
...
def get_patient_history(patient_id: int):
    ...
    for row in rows:
        ...
        try:
            ...
            item["result"] = parsed.get(
                "result",
                parsed,
            )

            item["fusion"] = parsed.get(
                "fusion"
            )

            item["severity"] = parsed.get(
                "severity"
            )

            item["treatment"] = parsed.get(
                "treatment"
            )

            item["ai_clinical_reasoning"] = (
                parsed.get(
